class Player extends Phaser.GameObjects.Container {
  constructor() {
    ...
    // this.radarBG = scene.add.sprite(20, 60, 'radarBack').setScrollFactor(0).setDepth(1000).setScale(1, 2).setOrigin(.5, 0.25).setFrame(2)
    this.radarBG = scene.add.sprite(20, 60, 'radarBack').setScrollFactor(0).setDepth(1000).setOrigin(.5, 0.25).setFrame(2)
  ...
  update(deltaMultiplier = 1) {
    ...
    if (this.jumpState === 0) {
      if (this.jumpedObjects > 0) {
        scene.score.bonus(this.hopScore(globalStats.currentRide.currentObjectsInRow));
        this.jumpedObjects = 0
      }
  ...
  onLaneSwitchMidpoint(newLane) {
    ...
    this.radarBG.setFrame(newLane === 2 ? 1 : 2)
  }
